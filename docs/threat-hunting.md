...
switch. The threat hunt sweeps that trail and surfaces the attacks, risk-ranked.
It runs inside the platform — the threat-hunter agent drives it (see below) —
and a hunt report looks like this:
...
The hunt sweep is the engine. The **threat-hunter agent** builds on it: it runs
...
The remediation engine assesses the deployment's security posture — control gaps
(from the compliance control map) plus active breach signals (from the hunt) —
and maps each gap to the fix that closes it.

Fixes split two ways:

- **Auto-fixable** — reversible, in-boundary flips of *Maverick's own* config
  (enable audit signing, set retention). Applied **only** under
  ...
  event, and the record tells you how to undo it.
