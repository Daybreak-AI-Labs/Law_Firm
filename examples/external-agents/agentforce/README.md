# Agentforce → Maverick gateway (paste-in Apex)

Bring a Salesforce Agentforce agent under Maverick governance with two
deployable invocable actions — no External Services registration required:

| File | Invocable action | Gateway call |
| --- | --- | --- |
| `MaverickGateway.cls` | **Maverick: Screen Action** (tool, detail, risk → allowed, rule, reason, requiresApproval, approvalId) | `POST /api/v1/external/screen` |
| `MaverickReportRun.cls` | **Maverick: Report Run** (title, outcome, costDollars, idempotencyKey → ok, goalId, overBudget, errorMessage) | `POST /api/v1/external/runs` |

Two classes because Apex permits exactly one `@InvocableMethod` per class;
`MaverickReportRun` reuses `MaverickGateway`'s shared HTTP/JSON plumbing.
Both call out through the Named Credential `Maverick_Gateway`, so the
...
1. **Setup → Security → Named Credentials → External Credentials** tab →
   **New**.
2. Label: `Maverick Gateway Auth` · Name: `Maverick_Gateway_Auth` ·
...
5. Grant access: **Setup → Users → Permission Sets** → **New**
   (e.g. `Maverick Gateway Access`) → on the permission set, open
   **External Credential Principal Access** → **Edit** → add
   `Maverick_Gateway_Auth - Agent` → Save. Assign the permission set to
...
1. **Setup → Security → Named Credentials → Named Credentials** tab →
   **New**.
2. Label: `Maverick Gateway` · Name: `Maverick_Gateway` — the API name
   **must** be exactly `Maverick_Gateway`; the Apex endpoint is
   `callout:Maverick_Gateway/api/v1/external/...`.
3. URL: your Maverick base URL, e.g. `https://maverick.example.com`
   (no trailing slash, no path — Apex appends `/api/v1/external/...`).
4. External Credential: `Maverick Gateway Auth`.
...
(`MaverickGateway` first — `MaverickReportRun` references it).
...
2. **This Topic's Actions → New → Add from Asset Library / Apex** →
   add **Maverick: Screen Action** and **Maverick: Report Run**
...
3. In the topic **Instructions**, add:
   ...
   > outside this org, spending money), run *Maverick: Screen Action*
   ...
   > retrying. When the job completes, run *Maverick: Report Run* exactly
...
```apex
MaverickGateway.ScreenRequest r = new MaverickGateway.ScreenRequest();
r.tool = 'send_email';
r.detail = 'smoke test from Agentforce setup';
r.risk = 'low';
System.debug(MaverickGateway.screen(new List<MaverickGateway.ScreenRequest>{ r })[0]);
```

Expect `allowed=true, rule=allow` (and an `external_action_screened` audit
event on the Maverick side). A readable `transport_error` reason tells you
...
- **Approval polling**: a parked approval is decided by a human in the
  Maverick dashboard queue; the row id is `approvalId`. To poll from
